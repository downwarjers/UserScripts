...
(function() {
    ...
    function scaleOne(canvas) {
        ...
        // 如果已經處理過，且倍率相同，則直接跳過。
        if (originalProperties.has(canvas) && originalProperties.get(canvas).scaleFactor === scaleFactor) {
            return;
        }

        // 如果該 canvas 尚未被劫持，我們就進行劫持
        if (!originalProperties.has(canvas)) {
            const originalWidthProp = Object.getOwnPropertyDescriptor(HTMLCanvasElement.prototype, 'width');
            const originalHeightProp = Object.getOwnPropertyDescriptor(HTMLCanvasElement.prototype, 'height');
            const originalGetContext = HTMLCanvasElement.prototype.getContext;

            // 劫持 width 屬性
            Object.defineProperty(canvas, 'width', {
                get: function() { return originalWidthProp.get.call(this); },
                set: function(val) { originalWidthProp.set.call(this, val * scaleFactor); },
                configurable: true
            });

            // 劫持 height 屬性
            Object.defineProperty(canvas, 'height', {
                get: function() { return originalHeightProp.get.call(this); },
                set: function(val) { originalHeightProp.set.call(this, val * scaleFactor); },
                configurable: true
            });
                ...
                if (context && args[0] === '2d') {
                    ...
                    context.setTransform = function(...transformArgs) {
                        ...
                        const newTransformArgs = [
                            transformArgs[0] * scaleFactor, transformArgs[1],
                            transformArgs[2], transformArgs[3] * scaleFactor,
                            transformArgs[4], transformArgs[5]
                        ];
